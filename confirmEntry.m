...
if(checkType==1)
    if(abs(cellEachFrame{1}{idx}.length - cellEachFrame{1}{idx}.outflow)<max([5,0.15*cellEachFrame{1}{idx}.length]) )
        flag=true;
        return;
    end
...
elseif(checkType==2)
    ...
    if(cellEachFrame{1}{idx}.length<LengthSkip*2)
        return
    end
    ...
    if(abs(cellEachFrame{1}{idx}.length - cellEachFrame{1}{idx}.outflow)...
            >max([5,0.15*cellEachFrame{1}{idx}.length]) )
        return;
    end
    ...
    if(abs(cellEachFrame{frameID}{cellID}.length - cellEachFrame{frameID}{cellID}.outflow)...
            <max([5,0.15*cellEachFrame{frameID}{cellID}.length]) )
        flag=true;
        return;
    end
