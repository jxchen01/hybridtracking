function [cellList, cellMat] = SortCellPixel(im)
...
while(~isempty(ep))
    ...
    cellList{currentCellNum}=struct('ctl',ctl,'length',pixNum,'child',[],'parent',[],'candi',[],'inflow',0,'outflow',0,'relaxinCost',0,'relaxOutCost',0);
    for i=1:1:pixNum
        cellMat(ctl(i,1),ctl(i,2))=currentCellNum;
    end
    clear tmpPixInd ctl flag pixNum
    ep=find(labelImg==2); 
end
